...
% - reproduces Fig 5 and 6 of paper
...
%% Figure 6: Strength of risk awareness and prevalence statistics
...
hold off; box off; grid off;
ylabel('mean infections', 'FontSize', fnt); xlim([min(n) max(n)]);
legend('', '', '', '', '', '', '', '', ['$\rho$ = ' num2str(rho(1))],...
    ['$\rho$ = ' num2str(rho(end))], fontsize=fnt);
...
hold off; box off; grid off;
ylabel('var of event R', 'FontSize', fnt); xlim([min(n) max(n)]);
...
hold off; box off; grid off;
ylabel('var of infections', 'FontSize', fnt); xlim([min(n) max(n)]);
xlabel(['sizes $n \, | \,$ risk strength $r = ' num2str(whet0) '$'], 'FontSize', fnt);
...
legend('', '', '', '', '', '', '', '', ['$r$ = ' num2str(whet(1))],...
    ['$r$ = ' num2str(whet(end))], fontsize=fnt);
...
hold off; box off; grid off;
xlim([min(n) max(n)]); 
xlabel(['sizes $n \, | \, $prevalence $\rho = ' num2str(rho0) '$'], 'FontSize', fnt);

%% Figure 5: Risk awareness and prevalence and superspreading
...
hold off; box off; grid off;
ylabel('infections ratio', 'FontSize', fnt);
xlim([min(n) max(n)]); 
xlabel(['sizes $n \, | \,$ risk strength $r = ' num2str(whet0) '$'], 'FontSize', fnt);
...
hold off; box off; grid off;
ylabel('tail infections prob, $\log$ P$(y \geq c)$', 'FontSize', fnt);
xlabel(['tail size $c \, | \, $event size $n = ' num2str(nval(1)) '$'], 'FontSize', fnt);
...
xlabel(['tail size $c \, | \, $event size $n = ' num2str(nval(2)) '$'], 'FontSize', fnt);
xlim([0 8]); h = gca; h.YLim = [-8 0];
